...
fixture_port_file="$work/plc-fixture.port"
...
cleanup() {
  for process in "$pid" "$fixture_pid"; do
...
for command in fwup python3 qemu-img qemu-system-x86_64 sftp ssh tailscale timeout; do
  require "$command"
done
...
mkdir -p "$work"
rm -f "$disk" "$console" "$qmp" "$log" "$qemu_log" "$fixture_port_file"

python3 "$root/integration/qemu/plc_fixture.py" "$fixture_port_file" \
...
plc_netdev="user,id=plc,net=192.168.10.0/24,restrict=on,guestfwd=tcp:192.168.10.100:10102-tcp:127.0.0.1:$fixture_port"
...
provisioned="$($root/integration/qemu/console.py \
  --socket "$console" \
  --log "$log" \
...
python3 "$root/integration/qemu/tcp_echo_client.py" "$tailnet_ip" 102
...
start_vm
booted="$($root/integration/qemu/console.py \
  --socket "$console" \
  --log "$log" \
